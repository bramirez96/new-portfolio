...
import { APIFetchOptions } from './API.types';
...
export default class API {
	...
	>(
		...
		try {
			...
			// Read out the content type of the response
			const contentType = response?.headers?.get('content-type');

			// Add handling for returning request response based on content type
			if (contentType) {
				if (contentType.indexOf('application/json') !== -1) {
					return response.json();
				} else {
					console.warn(`Unhandled content type received from fetch: ${contentType}`);
				}
			}

			// If there's no response, resolve with no value
			return undefined as ResponseType;
		} catch (err) {
			// Log out the error
			// TODO add a config check to see if logging is enabled, or do this with a logger class
			console.error('The following API request was unsuccessful and returned this error:');
			console.log('URL:', url);
	...
	>(
		...
		return this._Fetch<BodyType, ParamType, ResponseType>(path, apiFetchOpts);
	}
	 ...
	 */
	protected static _FormatFetchBody<
		BodyType = unknown,
		ParamType = unknown
	>(
